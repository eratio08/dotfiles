...
function contextModel(model: Model<Api>, mode: GptContextMode, lowContextWindow: number): Model<Api> {
...
function updateStatus(ctx: ExtensionContext, mode: GptContextMode, model: Model<Api> | undefined): void {
  ...
  ctx.ui.setStatus(STATUS_KEY, `${contextStatusLabel(ctx)}${contextModeEmoji(mode)}${active ? '' : ' (inactive)'}`)
}
...
export default function gptContextModeExtension(pi: ExtensionAPI): void {
  ...
  async function applyMode(model: Model<Api> | undefined, ctx: ExtensionContext): Promise<boolean> {
    ...
    const lowContextWindow =
      lowContextWindows.get(key) ?? GPT5_6_LOW_CONTEXT_WINDOWS.get(model.id) ?? model.contextWindow
