...
const todoSpecifier = specifiers.find(
	(specifier) => specifier.includes("todo") && specifier.endsWith("state.ts"),
);
assert.ok(
	todoSpecifier,
	"handoff/index.ts must dynamically import the todo state module",
);

const todoState = await import(
	pathToFileURL(resolve(handoffDir, todoSpecifier)).href
);
for (const name of [
...
const handoffTodos = todoState.getTodoHandoffSnapshot(
