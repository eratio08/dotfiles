---
description: Approve and merge safe Dependabot PRs from your review-requested inbox.
---
...
Do not edit dependency files, resolve conflicts, rerun CI, or push commits unless I explicitly ask.
...
Only all-patch or all-minor PRs can skip changelog research.

### 4. Approve minor and patch safe bumps

Completion criterion: every all-patch or all-minor safe bump has been approved unless already approved by someone else.
...
If PR is already approved by someone else, do not merge automatically.
Hand off before merge.
...
Merge only PRs approved by this run.
Do not merge PRs approved before this run by another reviewer.
Hand off instead.
Every visible check or status must be passing, skipped, or neutral.
...
Do not merge PRs not approved during this run.
Do not merge with pending, failing, missing, cancelled, or unknown checks.
Do not let failed checks block approval of `safe bump`.
They only block merge.
Do not edit dependency files, resolve conflicts, rerun CI, or push commits unless I explicitly ask.
