...
Since Opencode cannot run interactive TUI applications directly, this skill uses a tmux workaround:

1. Detects if Opencode is running inside tmux
2. If yes: Creates a split pane with tuicr running in it
3. If no: Provides instructions to restart Opencode inside tmux
...
2. **Run the wrapper script** with a 10-minute timeout:
   ```bash
   scripts/tuicr-wrapper.sh [directory]
   ```

   **IMPORTANT:** Always set `timeout: 600000` (10 minutes) on the Bash tool call.
   The script waits for tuicr to exit, and without the extended timeout Opencode will background the command after 2 minutes.
...
```bash
TUICR_PANE_SIZE=70 TUICR_PANE_POSITION=bottom scripts/tuicr-wrapper.sh /path/to/repo
```

## Example Invocations

User says: "review my changes"
→ Run scripts/tuicr-wrapper.sh with current working directory

User says: "let me review the changes in myproject"
→ Find myproject path based on context
→ Run scripts/tuicr-wrapper.sh with that path

User says: "/tuicr ~/projects/myapp"
→ Run scripts/tuicr-wrapper.sh with `~/projects/myapp`
